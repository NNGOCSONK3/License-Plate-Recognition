...
class ParkingApp:
    ...
    def capture_in(self):
        # THAY ĐỔI: Nút nhấn giờ là phương thức thủ công
        self.process_vehicle_entry(self.last_frame_in, rfid_uid="MANUAL_ENTRY")

    # HÀM MỚI: Logic xử lý xe vào, được gọi bởi cả nút nhấn và quét RFID
    def process_vehicle_entry(self, frame, rfid_uid):
        if frame is None:
            messagebox.showerror("Lỗi", "Không có tín hiệu từ camera vào.")
            return

        # Kiểm tra xem thẻ RFID đã được sử dụng cho xe khác trong bãi chưa
    ...
    def capture_out(self):
        ...
        # THAY ĐỔI: Thêm mã thẻ vào dữ liệu log
        log_data = {
            'ma_the': vehicle_data_in.get('rfid_uid', 'N/A'), # Lấy mã thẻ, nếu không có thì ghi 'N/A'
    ...
    def on_closing(self):
        if messagebox.askokcancel("Thoát", "Bạn có chắc muốn thoát chương trình?"):
            # THAY ĐỔI: Dừng luồng RFID và đóng cổng COM
            self.stop_thread.set()
            if self.serial_connection and self.serial_connection.is_open:
                self.serial_connection.close()
                print("Đã đóng cổng COM.")
            if self.rfid_thread:
                self.rfid_thread.join(timeout=1) # Chờ luồng kết thúc
    ...
    def open_settings_window(self):
        ...
        # Note: Logic to actually apply camera settings is omitted for brevity as it was in the original code.
        ...
        # THAY ĐỔI: Giao diện cài đặt cổng COM
        com_frame = self.create_labeled_frame(settings_window, "Kết nối đầu đọc RFID"); com_frame.pack(padx=20, pady=10, fill="x")
        com_port_var = tk.StringVar()
        tk.Label(com_frame, text="Cổng COM:", bg='#dcdad5').grid(row=0, column=0, padx=5, pady=5, sticky="w")
        com_combobox = ttk.Combobox(com_frame, textvariable=com_port_var, values=available_com_ports, state="readonly", width=15)
        com_combobox.grid(row=0, column=1, padx=5, pady=5)
        
        def connect_com_port():
            selected_port = com_port_var.get()
            if not selected_port or selected_port == available_com_ports[0]:
                messagebox.showerror("Lỗi", "Vui lòng chọn một cổng COM hợp lệ.")
                return
            self.start_rfid_listener(selected_port)
            messagebox.showinfo("Thành công", f"Đang lắng nghe trên cổng {selected_port}.")
            
        ttk.Button(com_frame, text="Kết nối", command=connect_com_port).grid(row=0, column=2, padx=10, pady=5)
    ...
    def log_vehicle_exit_to_csv(self, data, filename="lich_su_xe.csv"):
        ...
        try:
            # THAY ĐỔI: Thêm 'ma_the' vào danh sách trường
    ...
    def load_csv_data(self, filename, treeview, expected_cols):
        ...
        try:
            with open(filename, mode='r', newline='', encoding='utf-8') as f:
                ...
                try:
                    header = next(reader)
                    # Kiểm tra xem header có khớp không, đặc biệt quan trọng khi thêm cột mới
                    if len(header) != expected_cols:
                        print(f"Cảnh báo: File '{filename}' có cấu trúc cột không mong đợi.")
                        # Nếu file log cũ không có cột Mã thẻ, ta sẽ thêm một giá trị rỗng
                        ...
                        if is_log_file:
                            ...
                            for row in reader:
                                treeview.insert("", 0, values=['N/A'] + row) # Thêm cột giả
                        return
                ...
                for row in reader:
                    ...
                    if spot_id:
                        self.parking_spots[spot_id] = {
                            ...
                            'rfid_uid': 'RESERVED' # Gán mã thẻ mặc định cho xe đặt chỗ
                        }
    ...
    def schedule_display_reset(self):
        self.window.after(3000, self.reset_all_displays) # Tăng thời gian chờ lên 3s
    ...
    # --- CÁC HÀM MỚI CHO RFID ---
    def start_rfid_listener(self, com_port, baud_rate=9600):
        ...
        self.rfid_thread = threading.Thread(
            ...
            daemon=True # Luồng sẽ tự thoát khi chương trình chính thoát
        )
        self.rfid_thread.start()

    def read_from_serial_port(self, com_port, baud_rate):
        print(f"Bắt đầu luồng lắng nghe trên {com_port}...")
        try:
        ...
        while not self.stop_thread.is_set():
            try:
                if self.serial_connection.in_waiting > 0:
                    ...
                    if line:
                        # Decode từ bytes sang string, loại bỏ khoảng trắng thừa
                        uid = line.decode('utf-8').strip()
                        if uid:
                            print(f"RFID отсканирован: {uid}") # RFID отсканирован
                            self.rfid_queue.put(uid)
            except serial.SerialException:
                print(f"Lỗi đọc từ cổng {com_port}. Đang đóng kết nối.")
                break
            except Exception as e:
                print(f"Lỗi không xác định trong luồng RFID: {e}")
                break
        
        if self.serial_connection and self.serial_connection.is_open:
            self.serial_connection.close()
        print("Luồng lắng nghe RFID đã dừng.")

    def process_rfid_queue(self):
        try:
            # Lấy UID từ hàng đợi mà không block giao diện
            uid = self.rfid_queue.get_nowait()
            self.process_vehicle_entry(self.last_frame_in, rfid_uid=uid)
        except queue.Empty:
            # Hàng đợi rỗng, không làm gì cả
            pass
...
if __name__ == "__main__":
    root = tk.Tk()
    root.state('zoomed') 
    app = ParkingApp(root, "Hệ thống Quản lý Bãi giữ xe (Tích hợp RFID)")
